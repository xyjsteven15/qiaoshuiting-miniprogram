...
| 履约 | 到店再沟通 | 忌口备注前置、专属「小乔」客服直连、地址一键地图定位 |
...
4. **履约**：在「我的」查看预订；如需告知同行，可复制到店信息粘贴到群聊，或分享小程序卡片（点开即可导航）；如需沟通可一键致电「小乔」客服，或对餐厅地址一键地图定位导航到店。

### 3.3 订座关键业务规则

| 规则 | 说明 |
|------|------|
| 人数 ≤15 | 可选适配的独立包间：垂虹居（≤12）、羡鱼轩（≤12）、桥瑜汀（≤15）（以各厅后台 `min_guests` / `max_guests` 为准）；1-6 人另可选卡座、1-4 人另可选室外座位 |
| 人数 11-20 | 另可选大包间徽来堂 |
| 人数 >20 | 三个小包间隔断可拆卸，任意拼间连通（羡鱼轩+垂虹居 ≤24、桥瑜汀+垂虹居 ≤27、桥瑜汀+羡鱼轩 ≤27、三间全拼 ≤39）；**拼间不走线上下单，C 端引导致电门店预约** |
| 订满兜底 | 适配座位全部订满时：可留电话等位回电，或致电门店咨询拼间宴席 |
...
| 座位 | 属性 | 人数引导 | 特殊标注 |
|------|------|----------|----------|
| 卡座 | 开放座位（2 桌：4/6 人位） | 1-6 | C 端合并一张卡；≤4 人优先占 4 人位桌 |
| 室外座位 | 开放座位（4 桌均 4 人位） | 1-4 | C 端合并一张卡，显示余桌数 |
| 桥瑜汀 | 中小 | ≤15 | 隔断可拆卸，可拼间 |
| 羡鱼轩 | 中小 | ≤12 | 隔断可拆卸，可拼间 |
| 垂虹居 | 中小 | ≤12 | 隔断可拆卸，可拼间 |
| 徽来堂 | 大包 + KTV | 11-20 | 庆典 / 欢唱 |
| 拼间 | 两间/三间连通 | ≤39（三间全拼） | 容量为组成厅之和；转致电预约，不走线上 |
...
7. 提交成功页：订单摘要、寻店指引、发给同行（复制信息 / 分享到微信），跳转「我的」或返回首页
...
- 发给同行：复制到店信息（时间、人数、厅房、地址、寻店、电话）粘贴到群聊；或分享小程序卡片（点开进入公开到店页，可导航）
- 联系「小乔」客服（一键致电）
- 餐厅地址展示 + 地库寻店 + 一键地图定位（`wx.openLocation`）
- 企客/联系人信息（姓名、手机，订座时自动带出）

> 已移除：邀请函记录、私宴订单、偏好与服务（含「我的」页内氛围音开关，氛围音开关保留在首页顶栏）。「发给同行」不是定制邀请函，只转发公开的到店信息（不含预订编号与订座人手机号）。
...
1. 人数 ≤12 与 >12 时，可选包间池过滤正确；徽来堂始终展示 KTV 标识。  
2. 未支付定金无法完成锁位；超时 `held` 自动释放。  
3. 锁位方式仅「定金」，无尊享套餐等其他选项。  
4. 确认页仅「忌口清单」，无代客泊车 / 桌面布置。  
5. 全站无「私宴 / 专属订餐」入口，无「邀请函」入口（发给同行不是邀请函）。  
6. 「我的」页可正常致电「小乔」客服并对餐厅地址地图定位；进行中预订可复制到店信息或分享到微信。  
